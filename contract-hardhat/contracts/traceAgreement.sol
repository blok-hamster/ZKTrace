...
contract TraceAgreement {
    ...
    function verifyByOrder(bytes32[] memory _proof, bytes32 nullifier, bytes32 leaf) public  returns (bool) {
        ...
        bool complete = _checkVerificationState();
        emit Verified(signCount, verify);

        if(complete == true){
            ITraceAgreementFactory(factoryAddress).createInvoice(supplier, id, 1);
            ITraceAgreementFactory(factoryAddress).createInvoice(traceAdmin, id, 1);
        }
